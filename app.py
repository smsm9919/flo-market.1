...
def product_details(product_id):
    ...
    # Get chat messages for this product
    chat_messages = ChatMessage.query.filter_by(product_id=product_id).order_by(ChatMessage.timestamp).all()
    
    return render_template('product_details.html', product=product_dict, chat_messages=chat_messages)
...
def seller_inbox():
    ...
    # Mark all messages to this user as read when they view inbox
    try:
        from sqlalchemy import text
        update_query = text("""
            UPDATE messages 
            SET is_read = TRUE 
            WHERE recipient_email = :user_email AND is_read = FALSE
        """)
        ...
        db.session.commit()
        logger.info(f"Marked seller inbox messages as read for user {current_user.email}")
    except Exception as e:
...
def messages():
    ...
    try:
        ...
        # تحديث كل الرسائل اللي استلمها المستخدم إنها تم قراءتها
        update_query = text("""
        ...
        logger.info(f"Marked all received messages as read for user {current_user.email}")
        
        return render_template("messages.html", messages=messages_data)
        
    except Exception as e:
        logger.error(f"Messages view error: {str(e)}")
        db.session.rollback()
        flash('حدث خطأ في عرض الرسائل', 'error')
        return redirect(url_for('index'))
...
def seller_reply():
    ...
    try:
        ...
        # Create the reply entry for thread tracking in messages table
        ...
        # Find the original buyer ID from the original message
        # Since original_message.seller_id is the product owner, we need to find the buyer
        # We'll get the buyer from the original message buyer_email
        buyer_user = User.query.filter_by(email=original_message.buyer_email).first()
        
        # Also create entry in chat_messages table for real-time chat
        if buyer_user:  # Only create if we can find the buyer user
            chat_message = ChatMessage()
            chat_message.sender_id = current_user.id  # Current user (seller)
            chat_message.receiver_id = buyer_user.id  # The original buyer
            chat_message.product_id = product.id
...
فلوماركت
        ...
        # Save both reply record and chat message regardless of email status
        try:
            db.session.add(reply_record)
            if buyer_user:  # Only add chat message if buyer user exists
...
def unread_messages_count():
    """Get count of unread messages for current user - unified system"""
    try:
        # Count unread messages from ChatMessage table where current user is receiver
...
def send_message():
    ...
    try:
        ...
        if not product_id or not content or not receiver_id:
            return jsonify({'success': False, 'message': 'بيانات غير مكتملة'})
        
        # Get product info for validation
        product = Product.query.get_or_404(product_id)
        
        # Insert chat message using raw SQL for PostgreSQL compatibility
        from sqlalchemy import text
        query = text("""
            INSERT INTO chat_messages (sender_id, receiver_id, product_id, message, timestamp, is_read)
            VALUES (:sender_id, :receiver_id, :product_id, :message, CURRENT_TIMESTAMP, FALSE)
        ...
        logger.info(f"Chat message sent: {current_user.fullname} -> Product {product_id}: {content}")
...
def get_messages(product_id):
    ...
    try:
        ...
        db.session.execute(mark_read_query, {
            'user_id': current_user.id,
            'product_id': product_id
        })
        db.session.commit()
        
        # Get chat messages using raw SQL for PostgreSQL compatibility
        from sqlalchemy import text
        query = text("""
            SELECT cm.id, cm.sender_id, cm.receiver_id, cm.message, cm.timestamp, 
        ...
        messages_data = []
        for row in result:
            sender = User.query.get(row[1])
            messages_data.append({
                'id': row[0],
                'sender_id': row[1],
                'sender_name': sender.fullname if sender else 'مستخدم غير معروف',
                'content': row[3],
                'timestamp': row[4].strftime('%Y-%m-%d %H:%M'),
                'is_seller': (row[1] == product.user_id)
            })
        ...
        update_query = text("""
            UPDATE chat_messages 
            SET is_read = TRUE 
            WHERE product_id = :product_id AND receiver_id = :user_id
        """)
...
def get_chat_unread_count():
    ...
    try:
        ...
        logger.error(f"Chat unread count error: {str(e)}")
        return jsonify({'success': False, 'count': 0})

@app.route('/chat/<int:product_id>')
@login_required
def chat_page(product_id):
    """Dedicated chat page for a product"""
    try:
        product = Product.query.get_or_404(product_id)
        
        # Check if user has permission to chat about this product
        # Either they are the product owner or a potential buyer
        if current_user.id != product.user_id:
            # For buyers, check if they have at least one message with this product
            from sqlalchemy import text
            query = text("""
                SELECT COUNT(*) as count
                FROM chat_messages 
            ...
            # If no existing messages and user is not the seller, redirect to product page
            if message_count == 0:
                flash('ابدأ المحادثة من صفحة المنتج', 'info')
        ...
        flash('حدث خطأ في تحميل صفحة الدردشة', 'error')
...
# Simple message sending route
@app.route('/send_simple_message', methods=['POST'])
@login_required
def send_simple_message():
    """Simple form-based message sending"""
    try:
        product_id = request.form.get('product_id')
        receiver_id = request.form.get('receiver_id')
        content = request.form.get('content')
        
        if not product_id or not receiver_id or not content:
        ...
        new_message.product_id = product_id
        new_message.message = content.strip()
        ...
        logger.error(f"Send simple message error: {str(e)}")
        flash('خطأ في إرسال الرسالة', 'error')
        db.session.rollback()
